...
import org.eclipse.emf.ecore.{EClass, EOperation, EObject}
...
object Semantics {
  def deleteDeleted(resource: GraphResource)(implicit connection: Neo4j) = connection.transaction { implicit tx =>
    Cypher("""
      |MATCH (:Resource { id: { rootId } }) <-[:ORIGIN]- (:Layer) -[:hasA|parameter*0..4]-> (o{ $STATUS: "DELETED" })
  ...
  def propagate(resource: GraphResource)(implicit connection: Neo4j, task: TaskOutput) = connection.transaction { implicit tx =>
    Cypher(
      ...
      |ON MATCH SET l.$$STATUS = (CASE [l.name] WHEN [resource.id] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET l.name = resource.id, l.$$TYPE = "Layer"
      |
      |WITH resource, l
      |MATCH (resource) -[*]-> (origin:EObject { eClass: "http://www.eclipse.org/emf/2002/Ecore#//EClass" } )
      |MERGE origin <-[:ORIGIN]- (c:Class)
      |ON CREATE SET c.$$STATUS = "ADDED"
      |ON MATCH SET c.$$STATUS = (CASE [c.name] WHEN [origin.name] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET c.name = origin.name, c.$$TYPE = "Class"
      |MERGE l -[:hasA]-> c
      |
      |WITH origin as r, c
      ...
      |ON CREATE SET o.$$STATUS = "ADDED"
      |ON MATCH SET o.$$STATUS = (CASE [o.name] WHEN [origin.name] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET o.name = origin.name, o.$$TYPE = "Operation"
      |MERGE c -[:hasA]-> o
      |MERGE o -[:parameter]-> (p:Parameter { name: "this", position: 0, $$TYPE: "Parameter" }) -[:type]-> c
      |ON CREATE SET p.$$STATUS = "ADDED"
      |ON MATCH SET p.$$STATUS = "PRESERVED"
      |
      |WITH origin as oo, o
      ...
      |ON CREATE SET p.$$STATUS = "ADDED"
      |ON MATCH SET p.$$STATUS = (CASE [p.name, p.position] WHEN [origin.name, position] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET p.name = origin.name, p.position = position, p.$$TYPE = "Parameter"
    ...
    Cypher(
      ...
      |ON CREATE SET a.$$STATUS = "ADDED"
      |ON MATCH SET a.$$STATUS = (CASE [a.name] WHEN [origin.name] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET a.name = origin.name, a.$$TYPE = "Attribute"
    ...
    Cypher(
      ...
      |ON CREATE SET ref.$$STATUS = "ADDED"
      |ON MATCH SET ref.$$STATUS = (CASE [ref.name, ref.lowerBound, ref.upperBound] WHEN [origin.name, origin.lowerBound, origin.upperBound] THEN "PRESERVED" ELSE "MODIFIED" END)
      |SET ref.name = origin.name, ref.lowerBound = origin.lowerBound, ref.upperBound = origin.upperBound, ref.$$TYPE = "Reference"
    ...
    Cypher(
      ...
      |ON CREATE SET c.$$STATUS = "ADDED", c.proven = false
      |ON MATCH SET c.$$STATUS = (CASE [c.name, c.representation] WHEN [origin.name, origin.representation] THEN "PRESERVED" ELSE "MODIFIED" END),
      |             c.proven = (CASE c.representation WHEN origin.representation THEN c.proven ELSE false END)
      |SET c.name = origin.name, c.$$TYPE = "Constraint", c.representation = origin.representation
      |MERGE l -[:hasA]-> c
      |WITH l, c
      |MATCH l -[:IMPLEMENTS]-> l2 -[:hasA]-> (d:Constraint)
      |SET d.proven = (d.proven AND c.$$STATUS = "PRESERVED")""".stripMargin).execute()
    val refs = Cypher(
      s"""
      ...
      row.get[String]("to").filter(_ != "<null>").fold {
        s"""
         ...
         |MERGE (null:Null { name: "void", $$STATUS: "PRESERVED" }) -[:IMPLEMENTS]-> null
         |MERGE f -[r:$relation]-> null
         |ON CREATE SET r.$$STATUS = "ADDED"
         |ON MATCH SET r.$$STATUS = "PRESERVED"
         |RETURN NULL
        """.stripMargin
      } { (to: String) =>
        ...
        resource.getResourceSet().getEObject(uri,true) match {
          ...
          case other: EObject if other.eClass().getEStructuralFeature("name") != null =>
            ...
            s"""
             |MATCH f WHERE id(f) = $from
             |MERGE (null:External { name: "$name", uri: "$to", $$STATUS: "PRESERVED"  }) -[:IMPLEMENTS]-> null
             |MERGE f -[r:$relation]-> null
             |ON CREATE SET r.$$STATUS = "ADDED"
             |ON MATCH SET r.$$STATUS = "PRESERVED"
             |RETURN NULL""".stripMargin
          case other =>
            s"""
             |MATCH f WHERE id(f) = $from
             |MERGE (null:External { uri: "$to", $$STATUS: "PRESERVED"  }) -[:IMPLEMENTS]-> null
             |MERGE f -[r:$relation]-> null
             |ON CREATE SET r.$$STATUS = "ADDED"
             |ON MATCH SET r.$$STATUS = "PRESERVED"
             |RETURN NULL""".stripMargin
  ...
  def autoMap()(implicit connection: Neo4j) = connection.transaction { implicit tx =>
    Cypher("""
     ...
     |WHERE n.name = m.name
     |  AND n.$STATUS <> "DELETED" AND m.$STATUS <> "DELETED"
    ...
    Cypher("""
     ...
     |WHERE p.$STATUS <> "DELETED" AND q.$STATUS <> "DELETED"
     |  AND pt.$STATUS <> "DELETED" AND qt.$STATUS <> "DELETED"
    ...
    Cypher("""
     ...
     |  AND p.$STATUS <> "DELETED" AND q.$STATUS <> "DELETED"
     |  AND pt.$STATUS <> "DELETED" AND qt.$STATUS <> "DELETED"
     |CREATE UNIQUE p -[:IMPLEMENTS]-> q""".stripMargin).execute()
  }
  ...
  def listMappings()(implicit connection: Neo4j): Seq[Mapping] = connection.transaction { implicit tx =>
    Cypher("""
        ...
        |RETURN layerA.name as fromLayer, [a.$TYPE, a.name, classA.name, opA.name, at.name, a.stereotype] as from,
        |       layerB.name as toLayer, [b.$TYPE, b.name, classB.name, opB.name, bt.name, b.stereotype] as to
      """.stripMargin) {
      case row => Mapping(
  ...
  def ignoreModel(layer: String, model: LayerObject, reason: String)(implicit connection: Neo4j) = connection.transaction { implicit tx =>
    ...
    Cypher(s"""MATCH $nMatch SET n.$$IGNORE = "$reason"""").execute()
  }
  ...
  def findProblems()(implicit connection: Neo4j): Seq[SemanticIssue] = connection.transaction { implicit tx =>
    Cypher("""
      ...
      |WHERE ap.$STATUS <> "DELETED" AND bp.$STATUS <> "DELETED"
      |MATCH bp -[:hasA|parameter]-> b
      |WHERE b.$STATUS <> "DELETED" AND NOT EXISTS(b.$IGNORE)
      |  AND NOT () -[:IMPLEMENTS]-> b AND NOT b:Constraint
      |  AND NOT b:Requirement
      |MATCH b <-[:hasA|parameter*1..4]- (layerB:Layer)
      |WHERE layerB.$STATUS <> "DELETED"
      |OPTIONAL MATCH (classB:Class) -[:hasA|parameter*1..2]-> b
      |OPTIONAL MATCH (opB:Operation) -[:parameter]-> b
      |OPTIONAL MATCH b -[:type]-> bt
      |RETURN layerB.name as layer, [b.$TYPE, b.name, classB.name, opB.name, bt.name, b.stereotype] as model
    """.stripMargin) { row =>
      ...
      |  AND bt.$STATUS <> "DELETED" AND expected.$STATUS <> "DELETED"
      |  AND NOT expected.name IS NULL
      |MATCH a -[:type] -> actual
      |WHERE actual.$STATUS <> "DELETED"
      |  AND actual <> expected
    ...
    """.stripMargin) { row =>
      ...
      |MATCH (layerA:Layer) -[:hasA|parameter*1..4]-> a -[:IMPLEMENTS]-> (b { $STATUS: "DELETED" }) <-[:hasA|parameter*1..4]- (layerB:Layer)
      ...
      |OPTIONAL MATCH a -[:type]-> at
      |OPTIONAL MATCH b -[:type]-> bt
      |RETURN layerA.name as layer2, [a.$TYPE, a.name, classA.name, opA.name, at.name, a.stereotype] as implementation,
      |       layerB.name as layer, [b.$TYPE, b.name, classB.name, opB.name, bt.name, b.stereotype] as model
    """.stripMargin) { row =>
      ...
      |WHERE EXISTS (a.$IGNORE)
      |OPTIONAL MATCH (c:Class) -[:hasA|parameter*1..2]-> a
      |OPTIONAL MATCH (op:Operation) -[:parameter]-> a
      |OPTIONAL MATCH a -[:type]-> at
      |RETURN l.name as layer, [a.$TYPE, a.name, c.name, op.name, at.name, a.stereotype] as model, a.$IGNORE as reason
    """.stripMargin) { row =>
      IgnoredModel(row.get[String]("layer").get, row.get[LayerObject]("model").get,
                   row.get[String]("reason").get)
    } ++ Cypher("""
      |MATCH (layerA:Layer) -[:hasA|parameter*1..4]-> (a { $STATUS: "DELETED" }) -[:IMPLEMENTS]-> b <-[:hasA|parameter*1..4]- (layerB:Layer)
    ...
    """.stripMargin) { row =>
      ...
      |WHERE refA.$STATUS <> "DELETED" AND refB.$STATUS <> "DELETED"
      |  AND (refA.lowerBound <> refB.lowerBound OR refA.upperBound <> refB.upperBound)
      |MATCH refB <-[:hasA]- (classB:Class) <-[:hasA]- (layerB:Layer)
      |RETURN layerB.name as layer, [refB.$TYPE, refB.name, classB.name, NULL, NULL, NULL] as model,
