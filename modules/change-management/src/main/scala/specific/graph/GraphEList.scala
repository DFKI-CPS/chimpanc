...
import specific.secore.SEcore.SEObject
 ...
 * @since  20.11.2014
 */
class GraphEList[T] private [graph] (q: Seq[Node], f: Node => T)(implicit connection: Neo4j) extends EList[T] {
  val qi = Buffer(q:_*)
  ...
  def containsAll(x$1: java.util.Collection[_]): Boolean = buffer.containsAll(x$1)
  def get(x$1: Int): T = f(q(x$1))
  def indexOf(x$1: Any): Int = buffer.indexOf(x$1)
  def isEmpty(): Boolean = buffer.isEmpty
  def iterator(): java.util.Iterator[T] = q.map(f).iterator
  def lastIndexOf(x$1: Any): Int = buffer.lastIndexOf(x$1)
  def listIterator(x$1: Int): java.util.ListIterator[T] = buffer.listIterator(x$1)
  def listIterator(): java.util.ListIterator[T] = buffer.listIterator()
  ...
  def delete(x$1: Any): Boolean = x$1 match {
    case se: SEObject => remove(se.getObject)
    case g: GraphEObject => {
  ...
  def subList(x$1: Int,x$2: Int): java.util.List[T] = buffer.subList(x$1, x$2)
