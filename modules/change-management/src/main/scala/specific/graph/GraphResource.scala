 ...
 */
class GraphResource private[graph](val rootId: String, resourceSet: HybridResourceSet, loadOnDemand: Boolean)(implicit connection: Neo4j) extends Resource with GraphEContent {
  ...
  def append(obj: EObject): Boolean = connection.transaction { implicit tx =>
    ...
    for {
      t <- Option(connection.graphDb.findNode(DynamicLabel.label("Resource"), "id", rootId))
